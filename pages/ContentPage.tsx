...
const getYouTubeInfo = (url: string | undefined) => {
    if (!url) return null;
...
const ContentPage: React.FC<ContentPageProps> = ({ group, contentType }) => {
    ...
    const renderRepertorio = () => {
        const renderMusicDetails = (item: RepertorioItem) => {
            const videoInfo = getYouTubeInfo(item.videoUrl);
            const isPdf = item.sheetMusicUrl.toLowerCase().endsWith('.pdf');
            return (
                 <div className="space-y-4">
                    ...
                    {videoInfo && (
                        <div>
                            ...
                            <div className="relative cursor-pointer group w-full md:w-80" onClick={() => setModalVideoUrl(videoInfo.embedUrl)}>
                                <img src={videoInfo.thumbnailUrl} alt={`Thumbnail for ${item.title}`} className="rounded-lg shadow-md w-full" />
                    ...
                    <div>
                        ...
                        {isPdf ? (
                            <a href={item.sheetMusicUrl} target="_blank" rel="noopener noreferrer" className="inline-block bg-brand-gold text-brand-dark font-bold py-2 px-4 rounded-full hover:bg-yellow-500 transition-colors duration-300">
                                Abrir Partitura (PDF)
                            </a>
                        ) : (
                            <img src={item.sheetMusicUrl} alt={`Partitura de ${item.title}`} className="w-full h-auto rounded border" />
                        )}
                    </div>
                </div>
            );
